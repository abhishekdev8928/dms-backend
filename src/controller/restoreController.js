...
export const getTrashItems = async (req, res, next) => {
  try {
    ...
    // Only show deleted within last 30 days
    ...
    const [folders, totalFolders] = await Promise.all([
      FolderModel.find(baseQuery)
        .populate("updatedBy", "username email profilePic")
        .sort({ deletedAt: -1 })
        .lean(),
    ...
    const data = items.map(item => {
      ...
      const diffDays = Math.ceil(
        (autoDeleteDate - now) / (1000 * 60 * 60 * 24)
      );

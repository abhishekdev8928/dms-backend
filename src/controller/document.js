/** 
 * UPDATED DOCUMENT CONTROLLERS - Complete Activity Logging + File Validation
 ...
 * ✅ File validation using constants (allowed/blocked extensions and MIME types)
 */
...
/**
 * Generate presigned URLs for file upload with validation
...
export const generatePresignedUrls = async (req, res, next) => {
  try {
    ...
    if (validationErrors.length > 0) {
      throw createHttpError(400, "File validation failed", { 
    ...
    const urls = await Promise.all(
      files.map(async (file, index) => {
        ...
        return { 
          ...
          formatGroup: getFormatGroup(filename)
        };
      })
    );
...
/**
 * ✅ Create/Upload a new document with file validation
...
export const createDocument = async (req, res, next) => {
  try {
    ...
    const validation = validateFile(data.originalName, data.mimeType);
    if (!validation.valid) {
    ...
    res.status(201).json({
      ...
      data: {
        ...
        fileCategory: getFileCategory(sanitizedData.originalName),
        formatGroup: getFormatGroup(sanitizedData.originalName)
      },
    });
  } catch (error) {
    next(error);
...
export const findByExtension = async (req, res, next) => {
  try {
    ...
    if (!isAllowedExtension(`file${sanitizedExt}`)) {
      throw createHttpError(400, 'Extension not allowed');
    }
...
export const getDocumentById = async (req, res, next) => {
  try {
    ...
    const responseData = sanitizeObjectXSS({
      ...document.toJSON(),
      parent: parentInfo,
      fileCategory: getFileCategory(document.originalName || document.name),
      formatGroup: getFormatGroup(document.originalName || document.name)
    });
...
export const getDocumentsByParent = async (req, res, next) => {
  try {
    ...
    const sanitizedDocuments = documents.map(doc => {
      const docObj = sanitizeObjectXSS(doc.toObject());
      return {
        ...docObj,
        fileCategory: getFileCategory(doc.originalName || doc.name),
        formatGroup: getFormatGroup(doc.originalName || doc.name)
      };
    });
  ...
  try {
    ...
    res.status(200).json({
      ...
      data: {
        ...
        fileCategory: getFileCategory(downloadName),
        formatGroup: getFormatGroup(downloadName)
      },
    });
  } catch (error) {
    next(error);
...
/**
 * ✅ Create new version (Re-upload) with file validation
...
export const createVersion = async (req, res, next) => {
  try {
    ...
    // Validate file before creating version
    const validation = validateFile(data.originalName, data.mimeType);
    if (!validation.valid) {
      throw createHttpError(400, validation.reason);
    }
